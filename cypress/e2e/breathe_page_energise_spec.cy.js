describe("'Energise' Mode for Breathe Page", () => {
    beforeEach(() => {
    ...
    // Test case: Mode is 'energise' 
    it("should have a mode of 'energise'", () => {
        cy.get('#breathe-types').should('have.value', '3');
    });

    // Test case: Successfully transitions through breathe phases
    it('should transition through breathe phases', () => {
        cy.get('#container').click();
        cy.wait(5000); // Wait for countdown (3, 2, 1)
        cy.wait(3000); // Wait for breathe in phase
        cy.get('#text').should('have.text', 'Hold');
        cy.wait(1000); // Wait for hold phase
        cy.get('#text').should('have.text', 'One more sharp breath!');
        cy.get('#container').should('have.class', 'grow3_2');
        cy.wait(1000); // Wait for second breathe in phase
        ...
        cy.get('#container').should('have.class', 'grow3_1');
    });
    ...
    it('should redirect after completing the animation', () => {
        cy.get('#container').click();
        cy.wait(47000); // Wait for all cycles to finish
        cy.url().should('include', '/quote');
    });
});